...
def get_an_ark_server() -> ArkServer:
    data = {
        "status": "started",
    ...
    gameserver = GameServer(**data)
    return ArkServer(gameserver)


def test_ark_server():
    ark_server = get_an_ark_server()
    assert ark_server.service_id == 1
    assert ark_server.status == 'started'
    assert ark_server.location == 'US'
    assert ark_server.last_status_change == 1679636808
    assert ark_server.settings.service_id == 1
    assert ark_server.settings.config.service_id == 1
    assert ark_server.settings.features.service_id == 1
    assert ark_server.settings.general.service_id == 1
    assert ark_server.settings.gameini.service_id == 1


def test_ark_settings():
    ark_server = get_an_ark_server()
    settings = ark_server.settings
    assert settings.config.server_name == "[US] Flashy Thylacoleo hosted by nitrado.net"
    assert settings.config.disable_pvp == "False"
    assert settings.gameini.b_disable_genesis_missions == "False"
    assert settings.features.engine_settings == "False"
    assert settings.general.cross_play == "True"
    assert settings.start_param.pvp_structure_decay == "True"


def test_ark_query():
    ark_server = get_an_ark_server()
    query = ark_server.query
    assert query.map == "TheIsland"
    assert query.player_max == 20


def test_ark_game_specific():
    ark_server = get_an_ark_server()
    game_specific = ark_server.game_specific
    assert game_specific.path == "/games/ni1234567_1/noftp/arkxb/"
    assert game_specific.features.has_backups is True
