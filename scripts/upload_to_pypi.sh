# Need to have the PyPi token as an environment variable named 'PYPI'
# Ex.
# PYPI="1232abcd!@#$%^"
...
# Gets the version incremented by 1 revision number
function get_new_production_version () {
  # Gets the version from a 'setup.cfg' file and increments the revision number.
  # :param $1: The path to the file. If empty, default is local "./setup.cfg"
  ...
  _found=$(cat "$_path" | grep "production_version = .*")
...
# Updates the cfg file production_version
function update_cfg_production_version () {
  # Updates the cfg file's production_version number.
  # :param $1: The path to the local cfg file, else 'setup.cfg' is the default.
  # :param $2: The version number.
  # Example:
  #     update_cfg_version "setup.cfg" "1.2.3"

  _search="^production_version = [0-9]*\.[0-9]*\.[0-9]*$"
  _replace="production_version = $2"
  if [[ "$OSTYPE" == "linux-gnu" ]] | [[ "$OSTYPE" == "linux-gnu" ]] | [[ "$OSTYPE" == "darwin" ]]
  then
    if ! sudo sed -i "s/$_search/$_replace/" "$1" &>/dev/null
    then
...
function upload_to_pypi () {
  ...
  git add .
  git commit -m "Uploaded $3@$2 to PyPi"
  git push
}



# If any functions get_package_nane or get_new_production_version fails, exit the program

if ! get_package_name $_file &>/dev/null
then
  echo $(get_package_name $_file)
  return 1
elif ! get_new_production_version $_file &>/dev/null
then
  echo $(get_new_production_version $_file)
  return 1
fi


_version="$(get_new_production_version $_file)"
_package_name="$(get_package_name $_file)"

install_build
install_twine
update_cfg_production_version $_file $_version
update_cfg_version $_file $_version
upload_to_pypi $_file $_version $_package_name
