...
then
  ...
  echo y | python3 -m pip uninstall "$_package_name"
  python3 -m pip install -i https://test.pypi.org/simple/ "$_package_name"
  _version=$(pip freeze | grep "$_package_name*")
  printf "\n\033[32m$_version\033[0m\n"
elif [[ "$OSTYPE" == "msys" ]]
then
  # Lightweight shell and GNU utilities compiled for Windows (part of MinGW)
  echo y | python -m pip uninstall "$_package_name"
  python -m pip install -i https://test.pypi.org/simple/ "$_package_name"
  _version=$(pip freeze | grep "$_package_name.*")
  printf "\n\033[32m$_version\033[0m\n"
else
